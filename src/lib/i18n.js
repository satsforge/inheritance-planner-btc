...
const dict = {
  ...
  'plan.upgrade.vaultOption': {
    es: 'Contaste que tu heredero no sabría (o no sabés si sabría) ejecutar el proceso de acceso por su cuenta. Para ese caso puntual existe Bóveda de Herencia BTC (de este mismo grupo de herramientas): en vez de depender de que tu heredero sepa qué hacer en el momento, arma una dirección donde vos podés gastar en cualquier momento pero tu heredero puede reclamar su parte automáticamente después de un tiempo de espera que vos elegís — sin que nadie tenga que intervenir ni "avisar" nada. Es la herramienta de mayor riesgo del suite (usa un script no estándar), así que probala primero en testnet y leé su modelo de seguridad antes de usarla con fondos reales.',
    en: 'You mentioned your heir wouldn\'t know (or you\'re not sure they\'d know) how to carry out the access process on their own. For exactly that case there is BTC Inheritance Vault (from this same tool family): instead of depending on your heir knowing what to do at the time, it builds an address you can spend from at any time, while your heir can automatically claim their share after a waiting period you choose — with nobody needing to step in or "let anyone know." It is the highest-risk tool in the suite (it uses a non-standard script), so try it on testnet first and read its security model before using it with real funds.',
  },
