...
async function main() {
  ...
  const csp = [
    ...
    "img-src 'self' data:",
    "font-src 'self'",
    // This tool never checks a balance, never derives a Bitcoin key, and
    // never transmits anything - it only turns questionnaire answers into a
    // document, entirely in memory. No reason for it to reach any network
    // at all, unlike its watch-only siblings.
    "connect-src 'none'",
    "frame-src 'none'",
    "object-src 'none'",
    "base-uri 'none'",
